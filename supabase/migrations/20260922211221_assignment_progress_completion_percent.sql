-- Separate learner completion progress from score.
-- Submitted/manual-review work is 100% complete even when score is still pending.
-- In-progress custom exercises expose answer-based progress for admin/learner UX.
...
declare
  ...
  v_total integer := 0;
  v_done integer := 0;
  v_progress_total numeric := 0;
  v_resource_progress numeric := 0;
  v_question_count integer := 0;
  v_answered_count integer := 0;
  v_resource_completed_items integer := 0;
  v_resource_total_items integer := 0;
...
begin
  ...
  loop
    v_total := v_total + 1;
    v_state := 'not_started';
    v_resource_progress := 0;
    v_question_count := 0;
    v_answered_count := 0;
    v_resource_completed_items := 0;
    v_resource_total_items := 0;
    ...
    if v_resource.resource_type = 'custom_exercise' then
      ...
      if v_attempt.id is not null then
        select
          count(*),
          count(*) filter (where question.answered_at is not null)
      ...
      v_resource_total_items := v_question_count;
      v_resource_completed_items := case
        when v_state in ('completed', 'review') then v_question_count
        else v_answered_count
      end;
      v_resource_progress := case
        when v_state in ('completed', 'review') then 100
        when v_question_count > 0 then round(least(100, (v_answered_count::numeric / v_question_count::numeric) * 100), 1)
        else 0
      end;
      ...
      v_resource_completed_items := least(v_attempt_count, v_required);
      v_resource_total_items := v_required;
      v_resource_progress := round(least(100, (v_attempt_count::numeric / v_required::numeric) * 100), 1);
      ...
      v_resource_completed_items := coalesce(v_resource.collection_completed_items, 0);
      v_resource_total_items := coalesce(v_resource.collection_total_items, 0);
      v_resource_progress := case
        when v_state = 'completed' then 100
        when v_resource_total_items > 0 then round(
          least(100, (v_resource_completed_items::numeric / v_resource_total_items::numeric) * 100),
          1
        )
        else 0
      end;
    ...
    v_progress_total := v_progress_total + v_resource_progress;
    ...
    v_resources := v_resources || jsonb_build_array(jsonb_build_object(
      ...
      'state', v_state,
      'progress_percent', v_resource_progress,
      'score', case
        when v_resource.resource_type = 'custom_exercise' then v_progress_score
        else null
      end,
      'completed_items', v_resource_completed_items,
      'total_items', v_resource_total_items
    ));
  end loop;
  ...
  return jsonb_build_object(
    ...
    'progress_percent', case when v_total > 0 then round(v_progress_total / v_total, 1) else 0 end,
