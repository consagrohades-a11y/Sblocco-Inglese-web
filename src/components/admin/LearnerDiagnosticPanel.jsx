    ...
    <section className="rounded-2xl border border-ink/10 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-[#16211e]">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex rounded-full border border-ink/10 bg-linen/40 p-1 dark:border-white/10 dark:bg-white/[0.04]">
          <button type="button" onClick={() => setFilter('learning')} className={`rounded-full px-3 py-1.5 text-xs font-black ${filter === 'learning' ? 'bg-ink text-white dark:bg-emerald-300 dark:text-[#102019]' : 'text-ink/55 dark:text-white/55'}`}>Apprendimento</button>
          <button type="button" onClick={() => setFilter('precision')} className={`rounded-full px-3 py-1.5 text-xs font-black ${filter === 'precision' ? 'bg-ink text-white dark:bg-emerald-300 dark:text-[#102019]' : 'text-ink/55 dark:text-white/55'}`}>Precisione</button>
        </div>
      </div>

      {loading ? <p className="mt-5 text-sm font-bold text-ink/55 dark:text-white/55">Caricamento diagnostica...</p> : null}
      {error ? <p className="mt-5 text-sm font-bold text-red-700 dark:text-red-300">{error}</p> : null}
      {!loading && !error && visible.length === 0 ? <p className="mt-5 text-sm leading-6 text-ink/55 dark:text-white/55">Non ci sono ancora dati sufficienti in questa categoria.</p> : null}
          ...
          return (
            <article key={item.code} className="py-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div><p className="text-sm font-black text-ink dark:text-white">{item.label}</p><p className="mt-1 text-xs font-bold text-ink/45 dark:text-white/45">{item.code} · {item.topic}{item.subtopic ? ` · ${item.subtopic}` : ''}</p></div>
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-3">
                <div className="rounded-lg bg-linen/40 p-3 dark:bg-white/[0.04]"><p className="text-lg font-black text-ink dark:text-white">{rate}%</p><p className="text-xs font-bold text-ink/45 dark:text-white/45">errore totale</p></div>
                <div className="rounded-lg bg-linen/40 p-3 dark:bg-white/[0.04]"><p className="text-lg font-black text-ink dark:text-white">{recentRate}%</p><p className="text-xs font-bold text-ink/45 dark:text-white/45">ultimi 90 giorni</p></div>
                <div className="rounded-lg bg-linen/40 p-3 dark:bg-white/[0.04]"><p className="text-lg font-black text-ink dark:text-white">{Number(item.total_errors || 0)} / {Number(item.total_opportunities || 0)}</p><p className="text-xs font-bold text-ink/45 dark:text-white/45">errori e opportunità</p></div>
              </div>
            </article>
          );
        })}
