...
import { EXERCISE_MODES } from '../../lib/exerciseEngine.js';
...
export default function AssignmentPracticeEditor({ enabled, onEnabledChange, config, onChange }) {
  ...
  const available = useMemo(() => cards.filter((card) =>
    ...
    && (!config.batch || card.batch === config.batch)).length, [cards, config]);
    ...
    <section className="rounded-2xl border border-moss/25 bg-mint/15 p-5 shadow-sm dark:border-emerald-300/20 dark:bg-emerald-400/[0.06] sm:p-6">
      ...
      {enabled ? (
        <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <label><span className="text-xs font-black uppercase text-ink/60 dark:text-white/60">Numero di domande</span><select value={config.question_count} onChange={(event) => update('question_count', Number(event.target.value))} className={fieldClass}>{[5, 10, 15, 20].map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
          ...
          <p className="sm:col-span-2 lg:col-span-3 text-sm font-bold text-ink/60 dark:text-white/60">{loading ? 'Caricamento card pubblicate...' : error || `${available} card disponibili con questi filtri.`}</p>
        </div>
      ) : null}
    </section>
  );
