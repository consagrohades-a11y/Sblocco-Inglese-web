...
export default function AssignmentCollectionPicker({ value, onChange }) {
  ...
  return (
    <section className="rounded-2xl border border-ink/10 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-[#16211e] sm:p-8">
      ...
      {loading ? <p className="mt-5 text-sm font-bold text-ink/55 dark:text-white/55">Caricamento Collection...</p> : null}
      {!loading && !catalog.length ? <p className="mt-5 rounded-xl border border-dashed border-ink/15 p-4 text-sm text-ink/55 dark:border-white/15 dark:text-white/55">Nessuna Collection pubblicata. Pubblicane una dal catalogo Exercise Builder.</p> : null}
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        {catalog.map((item) => {
          ...
          return <button key={item.collectionVersionId} type="button" onClick={() => toggle(item)} className={`rounded-xl border p-4 text-left transition ${selected ? 'border-violet-500 bg-violet-50 dark:border-violet-300/40 dark:bg-violet-300/10' : 'border-ink/10 hover:bg-linen/40 dark:border-white/10 dark:hover:bg-white/[0.05]'}`}>
            ...
            <p className="mt-2 text-xs font-semibold text-ink/55 dark:text-white/55">{item.itemCount} tappe · {item.completionRule === 'percentage' ? `completa almeno il ${item.requiredPercent}%` : 'completa tutte le tappe'}</p>
          </button>;
        })}
      </div>
    </section>
