    ...
    <article className="rounded-2xl border border-ink/10 bg-white p-5 dark:border-white/10 dark:bg-white/[0.05]">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-xs font-semibold leading-5 text-ink/48 dark:text-white/48">{detail}</p>
        </div>
        <span className="text-2xl font-black text-moss dark:text-mint">{score}</span>
      </div>
      <div className="mt-4 h-2.5 overflow-hidden rounded-full bg-ink/8 dark:bg-white/10">
        <div className="h-full rounded-full bg-gradient-to-r from-moss via-mint to-butter" style={{ width: `${score}%` }} />
      </div>
    </article>
  );
}
    ...
    <div className="grid gap-6">
      ...
      {performance ? (
        <section className="rounded-[1.75rem] border border-ink/10 bg-paper p-6 shadow-soft dark:border-white/10 dark:bg-white/[0.035] sm:p-8">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.12em] text-coral">Performance osservata</p>
              <h2 className="mt-2 text-3xl font-black text-ink dark:text-white">{performance.observedLabel}</h2>
              <p className="mt-2 max-w-3xl text-sm font-semibold leading-7 text-ink/58 dark:text-white/58">
                Questo risultato deriva dalle risposte di lingua, dagli scenari e dai tre messaggi audio. L’autovalutazione influenza il percorso, ma non sostituisce la performance.
              </p>
            </div>
            <span className="text-5xl font-black text-moss dark:text-mint">{performance.observedScore}</span>
          </div>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            ...
            <PerformanceCard
              ...
              detail={`${performance.listeningCorrect} dettagli corretti su ${performance.listeningTotal}, ${performance.listeningReplays} ascolti totali`}
            />
            <PerformanceCard
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-[0.72fr_1.28fr]">
            <article className="rounded-2xl bg-ink p-5 text-white">
              <p className="text-xs font-black uppercase tracking-[0.1em] text-mint">Come ti sei valutato/a</p>
              <p className="mt-3 text-2xl font-black">{performance.selfReportedLabel}</p>
              <p className="mt-2 text-sm font-semibold text-white/55">Indice di autovalutazione: {performance.selfReportedScore}</p>
            </article>
            <article className="rounded-2xl border border-moss/20 bg-mint/35 p-5 dark:border-mint/20 dark:bg-mint/[0.08]">
              <p className="text-xs font-black uppercase tracking-[0.1em] text-moss dark:text-mint">Confronto</p>
              <h3 className="mt-3 text-xl font-black text-ink dark:text-white">{performance.alignmentTitle}</h3>
              <p className="mt-2 text-sm font-semibold leading-7 text-ink/65 dark:text-white/65">{performance.alignmentSummary}</p>
            </article>
          </div>
        </section>
      ) : null}
        ...
        <article className="rounded-[1.75rem] border border-ink/10 bg-white p-6 shadow-soft dark:border-white/10 dark:bg-white/[0.05] sm:p-7">
          ...
          <div className="mt-5 rounded-2xl bg-linen/70 p-4 dark:bg-white/[0.05]">
            ...
            <p className="mt-1 text-xs font-semibold leading-6 text-ink/58 dark:text-white/58">
              Mostra quanto una competenza appare disponibile in questa prova breve. Non è una certificazione linguistica ufficiale.
            </p>
          </div>
        </article>
      ...
      {emailStatus ? (
        <section className={`flex items-start gap-3 rounded-2xl border p-4 ${emailStatus === 'sent' ? 'border-emerald-200 bg-emerald-50 text-emerald-950 dark:border-emerald-300/20 dark:bg-emerald-400/[0.08] dark:text-emerald-100' : 'border-amber-200 bg-amber-50 text-amber-950 dark:border-amber-300/20 dark:bg-amber-400/[0.08] dark:text-amber-100'}`}>
          ...
          <div>
            <p className="text-sm font-black">{emailStatus === 'sent' ? 'Il risultato è stato inviato via email.' : 'Il risultato è salvo, ma l’email non è partita.'}</p>
            <p className="mt-1 text-xs font-semibold leading-5 opacity-75">
              {emailStatus === 'sent' ? 'Puoi riaprire l’analisi dal link contenuto nel messaggio.' : token ? 'Puoi conservare questa pagina e riprovare l’invio più tardi.' : 'Il risultato resta comunque visibile qui.'}
            </p>
          </div>
        </section>
      ) : null}
