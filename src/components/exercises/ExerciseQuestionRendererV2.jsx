...
function DialogueRoleplay({ question, answer, onChange, disabled }) {
  const content = question.content || {};
  ...
  function selectRole(role) { onChange({ role_key: role, turns: {} }); }
  function changeTurn(key, value) { onChange({ role_key: selectedRole, turns: { ...turns, [key]: value } }); }
  return <div className="grid gap-5">{content.scenario ? <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm font-semibold leading-6 text-amber-950 dark:border-amber-300/20 dark:bg-amber-300/[0.07] dark:text-amber-100">{content.scenario}</div> : null}<div><p className="text-xs font-black uppercase tracking-wide text-ink/45 dark:text-white/45">Scegli il tuo personaggio</p><div className="mt-3 grid gap-3 sm:grid-cols-2">{characters.filter((character) => character.selectable !== false).map((character) => <button key={character.key} type="button" disabled={disabled} onClick={() => selectRole(character.key)} className={`rounded-xl border p-4 text-left transition ${selectedRole === character.key ? 'border-violet-400 bg-violet-50 dark:border-violet-300/40 dark:bg-violet-400/10' : 'border-ink/10 bg-white dark:border-white/10 dark:bg-white/[0.04]'}`}><p className="font-black text-ink dark:text-white">{character.name}</p>{character.description ? <p className="mt-1 text-xs font-semibold leading-5 text-ink/50 dark:text-white/50">{character.description}</p> : null}</button>)}</div></div>{selectedCharacter ? <div className="grid gap-3">{(content.turns || []).map((turn, index) => { const speaker = characters.find((character) => character.key === turn.speaker); const learnerTurn = turn.speaker === selectedRole; return <div key={turn.key} className={`flex ${learnerTurn ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[88%] rounded-2xl border p-4 ${learnerTurn ? 'border-violet-300 bg-violet-50 dark:border-violet-300/30 dark:bg-violet-400/10' : 'border-ink/10 bg-linen/55 dark:border-white/10 dark:bg-white/[0.05]'}`}><p className="text-[0.68rem] font-black uppercase tracking-wide text-ink/40 dark:text-white/40">{speaker?.name || turn.speaker} · {index + 1}</p>{learnerTurn ? <textarea rows={3} value={turns[turn.key] || ''} onChange={(event) => changeTurn(turn.key, event.target.value)} disabled={disabled} placeholder={turn.prompt || 'Scrivi la battuta...'} className="focus-ring mt-2 w-full rounded-xl border border-violet-200 bg-white px-3 py-2 text-sm font-semibold text-ink dark:border-violet-300/20 dark:bg-[#101a17] dark:text-white" /> : <p className="mt-2 text-sm font-semibold leading-6 text-ink/75 dark:text-white/75">{turn.text || turn.prompt || 'Turno dell’altro personaggio.'}</p>}</div></div>; })}</div> : <p className="rounded-xl border border-dashed border-ink/15 p-4 text-sm font-semibold text-ink/50 dark:border-white/15 dark:text-white/50">Scegli un personaggio per iniziare il dialogo.</p>}<RubricPreview rubric={content.rubric} /></div>;
}
...
function AudioRecorder({ question, answer, onChange, disabled, attemptId, attemptQuestionId }) {
  ...
  const maxSeconds = Number(question.content?.max_seconds || 90);
  ...
  async function saveRecording() {
    ...
    setError('');
    try {
      const registered = await uploadExerciseAudioSubmission({ attemptId, attemptQuestionId, blob, durationSeconds: elapsed, previousAnswer: answer });
      onChange(registered);
  ...
  return <div className="grid gap-4">{question.content?.context ? <div className="rounded-xl border border-cyan-200 bg-cyan-50 p-4 text-sm font-semibold leading-6 text-cyan-950 dark:border-cyan-300/20 dark:bg-cyan-300/[0.07] dark:text-cyan-100">{question.content.context}</div> : null}{savedUrl ? <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-300/20 dark:bg-emerald-300/[0.07]"><p className="mb-3 flex items-center gap-2 text-sm font-black text-emerald-900 dark:text-emerald-100"><CheckCircle2 className="h-4 w-4" />Registrazione salvata</p><audio controls src={savedUrl} className="w-full" /></div> : null}{previewUrl ? <div className="rounded-xl border border-violet-200 bg-violet-50 p-4 dark:border-violet-300/20 dark:bg-violet-300/[0.07]"><p className="mb-3 text-sm font-black text-violet-900 dark:text-violet-100">Riascolta prima di salvare</p><audio controls src={previewUrl} className="w-full" /><div className="mt-3 flex flex-wrap gap-2"><button type="button" disabled={uploading || !attemptId} onClick={saveRecording} className="inline-flex items-center gap-2 rounded-full bg-violet-700 px-4 py-2 text-sm font-black text-white disabled:opacity-40"><Upload className="h-4 w-4" />{uploading ? 'Caricamento...' : 'Usa questa registrazione'}</button><button type="button" disabled={uploading} onClick={discardRecording} className="inline-flex items-center gap-2 rounded-full border border-violet-300 px-4 py-2 text-sm font-black text-violet-800 dark:text-violet-200"><RotateCcw className="h-4 w-4" />Registra di nuovo</button></div></div> : null}<div className="flex flex-wrap items-center gap-3">{recording ? <button type="button" onClick={stopRecording} className="inline-flex items-center gap-2 rounded-full bg-red-700 px-5 py-3 text-sm font-black text-white"><Square className="h-4 w-4 fill-current" />Ferma · {elapsed}s</button> : <button type="button" disabled={uploading} onClick={startRecording} className="inline-flex items-center gap-2 rounded-full bg-ink px-5 py-3 text-sm font-black text-white dark:bg-emerald-300 dark:text-[#102019]"><Mic className="h-4 w-4" />{savedUrl ? 'Registra di nuovo' : 'Inizia registrazione'}</button>}<span className="text-xs font-bold text-ink/45 dark:text-white/45">Massimo {maxSeconds} secondi</span></div>{!attemptId ? <p className="text-xs font-bold text-amber-700 dark:text-amber-200">La registrazione reale è disponibile soltanto durante un tentativo assegnato.</p> : null}{error ? <p className="flex items-start gap-2 rounded-lg border border-red-200 bg-red-50 p-3 text-sm font-bold text-red-900 dark:border-red-300/20 dark:bg-red-300/10 dark:text-red-100"><CircleAlert className="mt-0.5 h-4 w-4 shrink-0" />{error}</p> : null}<RubricPreview rubric={question.content?.rubric} /></div>;
}
...
export default function ExerciseQuestionRendererV2({
  ...
  const input = useMemo(() => {
    ...
    if (type === 'dialogue_roleplay') return <DialogueRoleplay question={question} answer={answer} onChange={onChange} disabled={disabled} />;
