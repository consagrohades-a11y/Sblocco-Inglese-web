...
function TextAnswer({ value, onChange, disabled, multiline = false, placeholder = 'Scrivi la risposta...' }) {
  const shared = 'focus-ring w-full rounded-xl border-2 border-ink/10 bg-white px-4 py-3 text-base font-semibold text-ink outline-none transition focus:border-moss dark:border-white/15 dark:bg-surface-800 dark:text-white';
        ...
        {(content.turns || []).map((turn, index) => (
          <div key={turn.key || index} className={`flex ${index % 2 ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[88%] rounded-2xl border p-4 ${index % 2 ? 'border-violet-200 bg-violet-50 dark:border-violet-300/20 dark:bg-violet-400/[0.07]' : 'border-cyan-200 bg-cyan-50 dark:border-cyan-300/20 dark:bg-cyan-400/[0.07]'}`}>
              <p className="text-[0.68rem] font-bold uppercase tracking-wide text-ink/60 dark:text-white/60">{turn.speaker}</p>
...
function TurnGuidance({ turn, recorded }) {
  ...
  return <div className="grid gap-3">
    {turn.objective ? <div><p className="text-xs font-bold uppercase tracking-wide text-violet-700 dark:text-violet-200">Il tuo obiettivo</p><p className="mt-1 text-sm font-semibold leading-6 text-ink/75 dark:text-white/75">{turn.objective}</p></div> : null}
