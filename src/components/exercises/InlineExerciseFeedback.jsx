...
export default function InlineExerciseFeedback({ item }) {
  ...
  if (item.correct) {
    return (
      <div className="mt-2 rounded-lg border border-moss/20 bg-mint/25 px-3 py-2 text-xs font-black text-ink">
        Corretto.
      </div>
    );
  }

  const hasAnswer = String(item.userAnswer || '').trim().length > 0;

  return (
    <div className="mt-2 rounded-lg border border-coral/25 bg-blush/70 px-3 py-2 text-xs leading-5 text-ink/75">
      <p className="font-black text-ink">{hasAnswer ? 'Non ancora.' : 'Manca la risposta.'}</p>
