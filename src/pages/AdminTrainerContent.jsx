...
const emptyCard = {
  id: '', public_id: '', canonical_text: '', italian_meaning: '',
  english_explanation: '', communicative_function: '', primary_context: '',
  level: 'A2', primary_domain: 'general', topic: '', priority: 'useful',
  register: 'neutral', usage_channel: 'both', tone: '',
  accepted_answers: [], pronunciation_ipa_us: '', pronunciation_learner_us: '',
  example_1: '', example_2: '', usage_note: '', collocations: [], tags: [],
  status: 'draft', review_status: 'pending', review_decision: '', review_notes: '',
};
...
const inputClass = 'w-full rounded-xl border border-ink/15 bg-white px-4 py-3 text-sm font-semibold text-ink outline-none focus:border-moss focus:ring-4 focus:ring-mint/40';
...
export default function AdminTrainerContent() {
  ...
  function newCard() {
    setSelected({ ...emptyCard, public_id: `general-${String(cards.length + 1).padStart(3, '0')}` });
    setMessage('');
    setError('');
  }
  ...
  async function saveCard(nextStatus = selected.status, nextReviewStatus = selected.review_status) {
    ...
    if (rpcError) {
      ...
      setMessage(nextStatus === 'published' ? 'Carta pubblicata.' : nextReviewStatus === 'approved' ? 'Carta approvata.' : 'Bozza salvata.');
      await loadCards();
    }
    setSaving(false);
  }
  ...
  return (
    <>
      <SEO title="Contenuti Trainer | Pannello admin | Sblocco Inglese" description="Crea, revisiona e pubblica le carte Trainer." />
        ...
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col gap-5 rounded-2xl border border-ink/10 bg-white p-6 shadow-soft sm:p-8 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-3xl text-base leading-7 text-ink/70">Le nuove carte vengono salvate direttamente in Supabase come bozze. Solo le carte complete e approvate possono essere pubblicate.</p>
            </div>
            <div className="flex flex-wrap gap-3">
              <button type="button" onClick={newCard} className="focus-ring min-h-11 rounded-full bg-ink px-5 py-2.5 text-sm font-black text-white hover:bg-moss">Nuova carta</button>
              <Link to="/admin" className="focus-ring inline-flex min-h-11 items-center rounded-full border border-ink/15 px-5 py-2.5 text-sm font-black text-ink">Pannello admin</Link>
            </div>
          </div>
          ...
          <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.45fr)_minmax(20rem,0.75fr)]">
            <form onSubmit={(event) => { event.preventDefault(); saveCard(); }} className="rounded-2xl border border-ink/10 bg-white p-5 shadow-sm sm:p-7">
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-ink/10 pb-5">
                <div>
                  <p className="text-xs font-black uppercase tracking-wide text-moss">{selected.id ? 'Modifica carta' : 'Nuova bozza'}</p>
                  <h2 className="mt-1 text-2xl font-black text-ink">{selected.canonical_text || 'Carta senza titolo'}</h2>
                </div>
              ...
              <div className="mt-6 grid gap-5 md:grid-cols-2">
                ...
                <Field label="Livello" required><select className={inputClass} value={selected.level} onChange={(e) => updateField('level', e.target.value)}><option>A2</option><option>B1</option><option>B2</option></select></Field>
                <Field label="Espressione inglese" required><input className={inputClass} value={selected.canonical_text} onChange={(e) => updateField('canonical_text', e.target.value)} /></Field>
                ...
                <Field label="Funzione comunicativa" required><input className={inputClass} value={selected.communicative_function} onChange={(e) => updateField('communicative_function', e.target.value)} placeholder="es. chiedere chiarimenti" /></Field>
                <Field label="Contesto principale" required><input className={inputClass} value={selected.primary_context} onChange={(e) => updateField('primary_context', e.target.value)} placeholder="es. conversazione quotidiana" /></Field>
                <Field label="Categoria"><input className={inputClass} value={selected.topic} onChange={(e) => updateField('topic', e.target.value)} /></Field>
              ...
              <div className="mt-5 grid gap-5">
                <Field label="Spiegazione d'uso" required><textarea rows="3" className={inputClass} value={selected.english_explanation} onChange={(e) => updateField('english_explanation', e.target.value)} /></Field>
                <div className="grid gap-5 md:grid-cols-2">
                  <Field label="Pronuncia IPA americana"><input className={inputClass} value={selected.pronunciation_ipa_us} onChange={(e) => updateField('pronunciation_ipa_us', e.target.value)} placeholder="/aɪm ɑn maɪ weɪ/" /></Field>
                ...
                <div className="grid gap-5 md:grid-cols-2">
                  <Field label="Collocazioni, una per riga"><textarea rows="3" className={inputClass} value={joinLines(selected.collocations)} onChange={(e) => updateField('collocations', splitLines(e.target.value))} /></Field>
              ...
              {!canPublish ? <p className="mt-3 text-xs font-bold leading-5 text-ink/50">Per pubblicare servono approvazione, almeno una risposta accettata, IPA americana, due esempi e una nota d'uso.</p> : null}
            </form>

            <aside className="rounded-2xl border border-ink/10 bg-white p-5 shadow-sm xl:sticky xl:top-24 xl:self-start">
              <h2 className="text-xl font-black text-ink">Carte in Supabase</h2>
              <div className="mt-4 grid gap-3">
                <input type="search" className={inputClass} value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Cerca inglese, italiano o ID" />
                <select className={inputClass} value={reviewFilter} onChange={(e) => setReviewFilter(e.target.value)}><option value="all">Tutte le revisioni</option><option value="pending">Da revisionare</option><option value="approved">Approvate</option><option value="rejected">Rifiutate</option></select>
              </div>
              <div className="mt-4 max-h-[42rem] divide-y divide-ink/10 overflow-y-auto rounded-xl border border-ink/10">
                ...
                {filteredCards.map((card) => (
                  <button key={card.id} type="button" onClick={() => openCard(card)} className="focus-ring block w-full p-4 text-left transition hover:bg-linen/50">
                    ...
                    <div className="mt-2 flex flex-wrap gap-2 text-[0.68rem] font-black uppercase tracking-wide text-ink/45"><span>{card.level}</span><span>{card.status}</span><span>{card.review_status}</span></div>
                  </button>
                ))}
              </div>
            </aside>
