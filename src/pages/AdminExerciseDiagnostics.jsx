...
function CodeEditor({ value, onChange, onSave, busy }) {
  ...
  return (
    <section className="rounded-2xl border border-ink/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-surface-900 sm:p-7">
      <div className="flex items-end justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-moss dark:text-emerald-300">Error code</p><h2 className="mt-1 text-xl font-black text-ink dark:text-white">{value.code || 'Nuovo codice'}</h2></div>{value.code ? <button type="button" onClick={() => onChange({ ...EMPTY_CODE })} className="text-xs font-black text-moss underline dark:text-emerald-300">Nuovo</button> : null}</div>
      ...
      <div className="mt-6 border-t border-ink/10 pt-5 dark:border-white/10">
        <p className="text-xs font-bold uppercase tracking-wide text-moss dark:text-emerald-300">Frasi predefinite</p>
      ...
      <button type="button" disabled={busy} onClick={onSave} className="mt-6 rounded-full bg-ink px-5 py-3 text-sm font-black text-white disabled:opacity-50 dark:bg-emerald-300 dark:text-surface-950">{busy ? 'Salvataggio...' : 'Salva codice e messaggi'}</button>
    </section>
  );
}
...
function RuleEditor({ value, onChange, onSave, busy }) {
  return (
    <section className="rounded-2xl border border-ink/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-surface-900 sm:p-7">
      <div className="flex items-end justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-violet-700 dark:text-violet-300">Aggregazione</p><h2 className="mt-1 text-xl font-black text-ink dark:text-white">{value.rule_key || 'Nuova regola'}</h2></div>{value.rule_key ? <button type="button" onClick={() => onChange({ ...EMPTY_RULE })} className="text-xs font-black text-moss underline dark:text-emerald-300">Nuova</button> : null}</div>
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
  ...
  return (
    <>
      ...
      <section className="section-shell py-8 lg:py-10"><div className="mx-auto max-w-7xl">
        <header className="rounded-2xl border border-ink/10 bg-white p-6 shadow-soft dark:border-white/10 dark:bg-surface-900 sm:p-8"><span className="eyebrow">Exercise Builder</span><div className="mt-4 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><h1 className="text-3xl font-black text-ink dark:text-white sm:text-4xl">Legenda diagnostica</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-ink/65 dark:text-white/65">I codici, le frasi e le regole vivono fuori dagli esercizi. Una regola generale può sostituire più avvisi specifici quando emerge una difficoltà sull’intero argomento.</p></div><div className="flex flex-wrap gap-2"><Link to="/admin/content/exercises/library" className="rounded-full border border-ink/15 bg-white px-4 py-2.5 text-sm font-black text-ink dark:border-white/20 dark:bg-white/10 dark:text-white">Libreria</Link><button type="button" disabled={busy} onClick={rebuild} className="rounded-full bg-ink px-4 py-2.5 text-sm font-black text-white disabled:opacity-50 dark:bg-emerald-300 dark:text-surface-950">Ricalcola storico</button></div></div></header>
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.3fr)_minmax(22rem,0.7fr)]">
          <section className="rounded-2xl border border-ink/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-surface-900 sm:p-7"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-wide text-moss dark:text-emerald-300">Codici registrati</p><h2 className="mt-1 text-2xl font-black text-ink dark:text-white">{codes.length}</h2></div><input value={search} onChange={(event) => setSearch(event.target.value)} className={`${fieldClass} mt-0 sm:max-w-xs`} placeholder="Cerca codice o topic" /></div>{loading ? <p className="mt-5 text-sm font-bold text-ink/65 dark:text-white/65">Caricamento...</p> : <div className="mt-5 divide-y divide-ink/10 border-y border-ink/10 dark:divide-white/10 dark:border-white/10">{filteredCodes.map((code) => <article key={code.code} className="flex flex-col gap-3 py-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><button type="button" onClick={() => editCode(code)} className="text-sm font-black text-moss underline dark:text-emerald-300">{code.code}</button><span className={`rounded-full px-2 py-1 text-[0.65rem] font-black ${code.category === 'precision' ? 'bg-violet-100 text-violet-800 dark:bg-violet-300/15 dark:text-violet-200' : 'bg-mint text-moss dark:bg-emerald-300/15 dark:text-emerald-200'}`}>{code.category}</span>{code.status === 'archived' ? <span className="rounded-full bg-slate-100 px-2 py-1 text-[0.65rem] font-black text-slate-600 dark:bg-white/10 dark:text-white/65">archiviato</span> : null}</div><p className="mt-1 text-sm font-bold text-ink dark:text-white">{code.label}</p><p className="mt-1 text-xs font-semibold text-ink/65 dark:text-white/65">{code.topic}{code.subtopic ? ` · ${code.subtopic}` : ''}{code.group_key ? ` · gruppo ${code.group_key}` : ''}</p></div><button type="button" onClick={async () => { await archiveDiagnosticCode(code.code, code.status !== 'archived'); await load(); }} className="self-start rounded-full border border-ink/15 px-3 py-1.5 text-xs font-black text-ink dark:border-white/20 dark:text-white">{code.status === 'archived' ? 'Riattiva' : 'Archivia'}</button></article>)}{filteredCodes.length === 0 ? <p className="py-6 text-sm text-ink/65 dark:text-white/65">Nessun codice corrispondente.</p> : null}</div>}</section>
          <section className="rounded-2xl border border-ink/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-surface-900 sm:p-7"><p className="text-xs font-bold uppercase tracking-wide text-violet-700 dark:text-violet-300">Regole attive</p><h2 className="mt-1 text-2xl font-black text-ink dark:text-white">{rules.length}</h2><div className="mt-5 divide-y divide-ink/10 border-y border-ink/10 dark:divide-white/10 dark:border-white/10">{rules.map((rule) => <article key={rule.rule_key} className="py-4"><div className="flex items-start justify-between gap-3"><div><button type="button" onClick={() => editRule(rule)} className="text-sm font-black text-violet-700 underline dark:text-violet-300">{rule.rule_key}</button><p className="mt-1 text-xs font-bold text-ink/65 dark:text-white/65">{rule.topic} · priorità {rule.priority}</p></div><button type="button" onClick={async () => { await archiveDiagnosticRule(rule.rule_key, rule.status !== 'archived'); await load(); }} className="text-xs font-black text-ink/60 underline dark:text-white/60">{rule.status === 'archived' ? 'Riattiva' : 'Archivia'}</button></div><p className="mt-3 text-sm leading-6 text-ink/65 dark:text-white/65">{rule.output_config?.messages?.it || 'Nessun messaggio italiano configurato.'}</p></article>)}{rules.length === 0 ? <p className="py-6 text-sm text-ink/65 dark:text-white/65">Nessuna regola configurata.</p> : null}</div></section>
        </div>
      </div></section>
    </>
  );
