...
function Field({ label, required = false, children }) {
  return (
    <label className="block">
      <span className="text-xs font-black uppercase tracking-wide text-ink/55 dark:text-white/65">{label}{required ? ' *' : ''}</span>
          ...
          <div className="grid gap-4 xl:grid-cols-[minmax(0,1.5fr)_minmax(20rem,0.72fr)]">
            <form onSubmit={(event) => { event.preventDefault(); saveCard(); }} className="rounded-2xl border border-ink/10 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-[#16211e] sm:p-5">
              ...
              <div className="mt-5 grid gap-3">
                <Field label="Definizione inglese" required><textarea rows="2" className={inputClass} value={selected.english_definition} onChange={(e) => updateField('english_definition', e.target.value)} /></Field>
                ...
                <Field label="Risposte accettate, una per riga"><textarea rows="2" className={inputClass} value={joinLines(selected.accepted_answers)} onChange={(e) => updateField('accepted_answers', splitLines(e.target.value))} /></Field>
                ...
                <Field label="Collocazioni, una per riga"><textarea rows="2" className={inputClass} value={joinLines(selected.common_collocations)} onChange={(e) => updateField('common_collocations', splitLines(e.target.value))} /></Field>
                <Field label="Nota d'uso"><textarea rows="2" className={inputClass} value={selected.usage_note || ''} onChange={(e) => updateField('usage_note', e.target.value)} /></Field>
                <Field label="Errore comune"><textarea rows="2" className={inputClass} value={selected.common_mistakes || ''} onChange={(e) => updateField('common_mistakes', e.target.value)} /></Field>
                <Field label="Tag, uno per riga"><textarea rows="2" className={inputClass} value={joinLines(selected.tags)} onChange={(e) => updateField('tags', splitLines(e.target.value))} /></Field>
                ...
                <Field label="Note di revisione"><textarea rows="2" className={inputClass} value={selected.review_notes || ''} onChange={(e) => updateField('review_notes', e.target.value)} /></Field>
              </div>
                    ...
                    return (
                      <div key={card.id} className="flex items-stretch transition hover:bg-linen/50 dark:hover:bg-white/5">
                        ...
                        <button type="button" onClick={() => openCard(card)} className="focus-ring block min-w-0 flex-1 p-4 text-left">
                          ...
                          <div className="mt-2 flex flex-wrap gap-2 text-[0.68rem] font-black uppercase tracking-wide text-ink/45 dark:text-white/45"><span>{card.public_id}</span><span>{card.level}</span><span>{card.status}</span><span>{card.review_status}</span></div>
                        </button>
                      </div>
                    );
                  })}
