...
function DialogueChoiceEditor({ question, setQuestion, codes }) {
  ...
  const updateTurn = (index, patch) => updateContent({ turns: turns.map((turn, current) => current === index ? { ...turn, ...patch } : turn) });
  return <div className="grid gap-5"><label className="text-xs font-black">Scenario<textarea rows={3} value={content.scenario || ''} onChange={(event) => updateContent({ scenario: event.target.value })} className={fieldClass} /></label><div className="grid gap-3"><p className="text-xs font-black uppercase tracking-wide text-violet-700">Turni visibili</p>{turns.map((turn, index) => <div key={turn.key || index} className="grid gap-3 rounded-xl border border-violet-200 p-4 dark:border-violet-300/20 sm:grid-cols-[12rem_minmax(0,1fr)_auto] sm:items-end"><label className="text-xs font-black">Personaggio<input value={turn.speaker || ''} onChange={(event) => updateTurn(index, { speaker: event.target.value })} className={fieldClass} /></label><label className="text-xs font-black">Battuta<input value={turn.text || ''} onChange={(event) => updateTurn(index, { text: event.target.value })} className={fieldClass} /></label><button type="button" disabled={turns.length <= 2} onClick={() => updateContent({ turns: turns.filter((_, current) => current !== index) })} className="pb-3 text-xs font-black text-red-700 underline disabled:opacity-30">Rimuovi</button></div>)}<button type="button" onClick={() => updateContent({ turns: [...turns, { key: `turn_${turns.length + 1}`, speaker: '', text: '' }] })} className="justify-self-start rounded-full border border-violet-300 px-4 py-2 text-xs font-black text-violet-800">Aggiungi turno</button></div><label className="text-xs font-black">Domanda finale<input value={content.response_prompt || ''} onChange={(event) => updateContent({ response_prompt: event.target.value })} className={fieldClass} /></label><OptionEditor question={question} setQuestion={setQuestion} codes={codes} /></div>;
}
...
function DialogueEditor({ question, setQuestion }) {
  ...
  return <div className="grid gap-5">
    <label className="text-xs font-black">Scenario<textarea rows={3} value={content.scenario || ''} onChange={(event) => updateContent({ scenario: event.target.value })} className={fieldClass} /></label>
    <label className="text-xs font-black">Modalità di risposta<select value={content.response_mode || 'written'} onChange={(event) => updateContent({ response_mode: event.target.value })} className={fieldClass}><option value="written">Testo per ogni turno</option><option value="audio">Audio legacy (compatibilità)</option><option value="audio_per_turn">Audio separato per ogni turno</option></select></label>
    <section>
      <div className="flex items-center justify-between"><p className="text-xs font-black uppercase tracking-wide text-violet-700">Personaggi</p><button type="button" onClick={() => updateContent({ characters: [...characters, { key: `character_${characters.length + 1}`, name: `Personaggio ${characters.length + 1}`, description: '', selectable: true }] })} className="text-xs font-black text-violet-700 underline">Aggiungi</button></div>
      <div className="mt-3 grid gap-3">{characters.map((character, index) => <div key={`${character.key}-${index}`} className="grid gap-3 rounded-xl border border-ink/10 p-4 sm:grid-cols-[10rem_12rem_minmax(0,1fr)_auto_auto] sm:items-end"><label className="text-xs font-black">Chiave<input value={character.key || ''} onChange={(event) => updateCharacter(index, { key: event.target.value })} className={fieldClass} /></label><label className="text-xs font-black">Nome<input value={character.name || ''} onChange={(event) => updateCharacter(index, { name: event.target.value })} className={fieldClass} /></label><label className="text-xs font-black">Descrizione<input value={character.description || ''} onChange={(event) => updateCharacter(index, { description: event.target.value })} className={fieldClass} /></label><label className="flex items-center gap-2 pb-3 text-xs font-black"><input type="checkbox" checked={character.selectable !== false} onChange={(event) => updateCharacter(index, { selectable: event.target.checked })} />Selezionabile</label><button type="button" disabled={characters.length <= 2} onClick={() => updateContent({ characters: characters.filter((_, current) => current !== index) })} className="pb-3 text-xs font-black text-red-700 underline disabled:opacity-30">Rimuovi</button></div>)}</div>
    </section>
    <section>
      <div className="flex items-center justify-between"><p className="text-xs font-black uppercase tracking-wide text-violet-700">Turni</p><button type="button" onClick={() => updateContent({ turns: [...turns, { key: `turn_${turns.length + 1}`, speaker: characters[0]?.key || '', text: '', prompt: '', learner_response: true, required: true, constraints: { min_seconds: 5, max_seconds: 35 } }] })} className="text-xs font-black text-violet-700 underline">Aggiungi</button></div>
      <div className="mt-3 grid gap-3">{turns.map((turn, index) => <div key={`${turn.key}-${index}`} className="rounded-xl border border-ink/10 p-4"><div className="grid gap-3 sm:grid-cols-[10rem_12rem_auto_auto] sm:items-end"><label className="text-xs font-black">Chiave<input value={turn.key || ''} onChange={(event) => updateTurn(index, { key: event.target.value })} className={fieldClass} /></label><label className="text-xs font-black">Parla<select value={turn.speaker || ''} onChange={(event) => updateTurn(index, { speaker: event.target.value })} className={fieldClass}>{characters.map((character) => <option key={character.key} value={character.key}>{character.name || character.key}</option>)}</select></label><label className="flex items-center gap-2 pb-3 text-xs font-black"><input type="checkbox" checked={Boolean(turn.learner_response)} onChange={(event) => updateTurn(index, { learner_response: event.target.checked })} />Risposta studente</label><button type="button" disabled={turns.length <= 2} onClick={() => updateContent({ turns: turns.filter((_, current) => current !== index) })} className="pb-3 text-xs font-black text-red-700 underline disabled:opacity-30">Rimuovi</button></div><div className="mt-3 grid gap-3 sm:grid-cols-2"><label className="text-xs font-black">Battuta fissa dell’altro personaggio<textarea rows={2} value={turn.text || ''} onChange={(event) => updateTurn(index, { text: event.target.value })} className={fieldClass} /></label><label className="text-xs font-black">Indicazione per la battuta dello studente<textarea rows={2} value={turn.prompt || ''} onChange={(event) => updateTurn(index, { prompt: event.target.value })} className={fieldClass} /></label></div>{audioPerTurn && turn.learner_response ? <DialogueTurnAudioEditor turn={turn} onChange={(patch) => updateTurn(index, patch)} /> : null}</div>)}</div>
    </section>
    <RubricEditor rubric={content.rubric || []} onChange={(rubric) => updateContent({ rubric })} />
  </div>;
}
...
function DialogueTurnAudioEditor({ turn, onChange }) {
  ...
  const updateConstraints = (patch) => onChange({ constraints: { ...constraints, ...patch } });
  const lines = (value) => String(value || '').split('\n').map((item) => item.trim()).filter(Boolean);
  return <div className="mt-4 grid gap-3 rounded-xl border border-violet-200 bg-violet-50/40 p-4 dark:border-violet-300/20 dark:bg-violet-400/[0.05]">
    <label className="flex items-center gap-2 text-xs font-black"><input type="checkbox" checked={turn.required !== false} onChange={(event) => onChange({ required: event.target.checked })} />Turno obbligatorio</label>
    <div className="grid gap-3 sm:grid-cols-3"><label className="text-xs font-black">Direzione<input value={turn.direction || ''} onChange={(event) => onChange({ direction: event.target.value })} className={fieldClass} /></label><label className="text-xs font-black">Contesto aggiuntivo<input value={turn.context || ''} onChange={(event) => onChange({ context: event.target.value })} className={fieldClass} /></label><label className="text-xs font-black">Obiettivo comunicativo<input value={turn.objective || ''} onChange={(event) => onChange({ objective: event.target.value })} className={fieldClass} /></label></div>
    <div className="grid gap-3 sm:grid-cols-2"><label className="text-xs font-black">Suggerimento prima della registrazione<input value={turn.hint || ''} onChange={(event) => onChange({ hint: event.target.value })} className={fieldClass} /></label><label className="text-xs font-black">Suggerimento dopo il primo tentativo<input value={turn.retry_hint || ''} onChange={(event) => onChange({ retry_hint: event.target.value })} className={fieldClass} /></label></div>
    <div className="grid gap-3 sm:grid-cols-2"><label className="text-xs font-black">Durata minima, secondi<input type="number" min="0" value={constraints.min_seconds ?? 0} onChange={(event) => updateConstraints({ min_seconds: Number(event.target.value) || 0 })} className={fieldClass} /></label><label className="text-xs font-black">Durata massima, secondi<input type="number" min="5" value={constraints.max_seconds ?? 35} onChange={(event) => updateConstraints({ max_seconds: Number(event.target.value) || 35 })} className={fieldClass} /></label></div>
    <div className="grid gap-3 sm:grid-cols-2"><label className="text-xs font-black">Punti obbligatori, uno per riga<textarea rows={3} value={(constraints.required_points || []).join('\n')} onChange={(event) => updateConstraints({ required_points: lines(event.target.value) })} className={fieldClass} /></label><label className="text-xs font-black">Espressioni consigliate, una per riga<textarea rows={3} value={(constraints.recommended_language || []).join('\n')} onChange={(event) => updateConstraints({ recommended_language: lines(event.target.value) })} className={fieldClass} /></label><label className="text-xs font-black">Espressioni obbligatorie, una per riga<textarea rows={3} value={(constraints.required_language || []).join('\n')} onChange={(event) => updateConstraints({ required_language: lines(event.target.value) })} className={fieldClass} /></label><label className="text-xs font-black">Espressioni da evitare, una per riga<textarea rows={3} value={(constraints.avoid_language || []).join('\n')} onChange={(event) => updateConstraints({ avoid_language: lines(event.target.value) })} className={fieldClass} /></label></div>
  </div>;
}
