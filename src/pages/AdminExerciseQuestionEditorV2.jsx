...
import { normalizeWordOrderAuthoringContent } from "../lib/wordOrderPresentation.js";
...
const TYPE_GROUPS = [
  [
    ...
    [
      ...
      ["dialogue_choice", "Dialogo con scelta della risposta"],
...
function rubricTotal(rubric) {
  return (
    (rubric || []).reduce(
      (sum, item) => sum + Number(item.max_points || 0),
      0,
    ) || 1
  );
}
...
function defaultRubric(kind = "writing") {
  ...
  if (kind === "interaction")
    return [
      {
        key: "interaction",
        label: "Gestione del dialogo",
        description: "",
        max_points: 4,
      },
      {
        key: "functional_language",
        label: "Funzioni comunicative",
        description: "",
        max_points: 3,
      },
...
function defaultContent(type) {
  ...
  if (type === "dialogue_choice")
    return {
      ...
      response_prompt: "",
      turns: [
        { key: "turn_1", speaker: "Personaggio 1", text: "" },
        { key: "turn_2", speaker: "Personaggio 2", text: "" },
      ],
      options: [
        { key: uid("opt"), text: "", is_correct: true, error_code: null },
        { key: uid("opt"), text: "", is_correct: false, error_code: null },
  ...
  if (type === "gap_fill")
    return {
      text_template: "[[blank_1]]",
      blanks: [{ key: "blank_1", accepted_answers: [""], points: 1 }],
    };
  if (type === "select_gap")
    return {
      text_template: "[[blank_1]]",
      ...
      turns: [
        {
          ...
          speaker: "character_1",
          text: "",
          prompt: "Scrivi la prima battuta.",
        ...
        },
        {
          ...
          speaker: "character_2",
          text: "",
          prompt: "Rispondi in modo appropriato.",
          learner_response: true,
        },
      ],
...
function defaultSkill(type) {
  if (type === "dialogue_choice") return "functional_language";
...
function emptyQuestion(type = "multiple_choice") {
  return {
    ...
    questionType: type,
    title: "",
    prompt: type === "content_block" ? "Prima di iniziare" : "",
    instructions: "",
    instructionLanguage: "it",
    level: "A1",
    topic: "",
    subtopic: "",
    primarySkill: defaultSkill(type),
    learningObjective: "",
    difficulty: "standard",
    tagsText: type === "content_block" ? "spiegazione, scaffolding" : "",
...
function fromDetail(detail) {
  return {
    ...
    primarySkill: detail.primary_skill || defaultSkill(detail.question_type),
...
function OptionEditor({ question, setQuestion, codes }) {
  ...
  function update(index, patch) {
    let next = options.map((option, current) =>
      current === index ? { ...option, ...patch } : option,
    );
    if (
      patch.is_correct &&
      ["multiple_choice", "dialogue_choice"].includes(question.questionType)
    )
      next = next.map((option, current) => ({
        ...option,
        is_correct: current === index,
      }));
    setQuestion({
      ...question,
      content: { ...question.content, options: next },
    });
  }
  return (
    <div className="grid gap-3">
      {options.map((option, index) => (
        <div
          key={option.key}
          className="grid gap-3 rounded-xl border border-ink/10 p-4 dark:border-white/10 sm:grid-cols-[auto_minmax(0,1fr)_13rem_auto] sm:items-end"
        >
          <label className="flex items-center gap-2 pb-3 text-xs font-black">
            <input
              type={
                ["multiple_choice", "dialogue_choice"].includes(
              ...
              onChange={(event) =>
                update(index, { is_correct: event.target.checked })
              }
            />
            Corretta
          </label>
          ...
          <label className="text-xs font-black text-ink/60 dark:text-white/60">
            Errore se scelta
            <select
              value={option.error_code || ""}
              onChange={(event) =>
                update(index, { error_code: event.target.value || null })
              }
              className={fieldClass}
            >
          ...
          <button
            ...
            className="pb-3 text-xs font-black text-red-700 underline disabled:opacity-30 dark:text-red-300"
          >
            Rimuovi
          </button>
        </div>
      ...
      <button
        ...
        className="justify-self-start rounded-full border border-ink/15 px-4 py-2 text-xs font-black dark:border-white/20"
      >
        Aggiungi opzione
      </button>
    </div>
...
function DialogueChoiceEditor({ question, setQuestion, codes }) {
  ...
  const updateTurn = (index, patch) =>
    updateContent({
      turns: turns.map((turn, current) =>
        current === index ? { ...turn, ...patch } : turn,
      ),
    });
  return (
    <div className="grid gap-5">
      <label className="text-xs font-black">
        ...
        <textarea
          ...
          onChange={(event) => updateContent({ scenario: event.target.value })}
          className={fieldClass}
        />
      </label>
      <div className="grid gap-3">
        ...
        {turns.map((turn, index) => (
          <div
            key={turn.key || index}
            className="grid gap-3 rounded-xl border border-violet-200 p-4 dark:border-violet-300/20 sm:grid-cols-[12rem_minmax(0,1fr)_auto] sm:items-end"
          >
            <label className="text-xs font-black">
              Personaggio
              <input
                value={turn.speaker || ""}
                onChange={(event) =>
                  updateTurn(index, { speaker: event.target.value })
                }
                className={fieldClass}
              />
            </label>
            <label className="text-xs font-black">
              ...
              <input
                value={turn.text || ""}
                onChange={(event) =>
                  updateTurn(index, { text: event.target.value })
                }
                className={fieldClass}
              />
            </label>
            <button
              ...
              onClick={() =>
                updateContent({
                  turns: turns.filter((_, current) => current !== index),
                })
              }
              className="pb-3 text-xs font-black text-red-700 underline disabled:opacity-30 dark:text-red-300"
            >
        ...
        <button
          ...
          onClick={() =>
            updateContent({
              turns: [
                ...turns,
                { key: `turn_${turns.length + 1}`, speaker: "", text: "" },
              ],
            })
          }
          className="justify-self-start rounded-full border border-violet-300 px-4 py-2 text-xs font-black text-violet-800 dark:text-violet-200"
        >
          Aggiungi turno
        </button>
      </div>
      <label className="text-xs font-black">
        Domanda finale
        <input
          value={content.response_prompt || ""}
          onChange={(event) =>
            updateContent({ response_prompt: event.target.value })
          }
          className={fieldClass}
        />
      </label>
...
function BlankEditor({ question, setQuestion }) {
  ...
  const updateContent = (patch) =>
    setQuestion({ ...question, content: { ...question.content, ...patch } });
  const update = (index, patch) => {
    const previousKey = blanks[index]?.key;
    const nextBlanks = blanks.map((blank, current) =>
      current === index ? { ...blank, ...patch } : blank,
    );
    const nextTemplate =
  ...
  const remove = (index) => {
    ...
    updateContent({
      ...
      text_template: removedKey
        ? template.replaceAll(`[[${removedKey}]]`, "")
        : template,
    });
  };
  const add = () => {
    ...
    updateContent({
      blanks: [
        ...blanks,
        {
          key,
          accepted_answers: [""],
          ...(question.questionType === "select_gap"
            ? { options: ["", ""] }
            : {}),
          points: 1,
        },
      ...
      <div className="rounded-xl border border-cyan-200 bg-cyan-50/45 p-4 dark:border-cyan-300/20 dark:bg-cyan-400/[0.05]">
        <label className="text-xs font-black">
          ...
          <textarea
            ...
            value={template}
            onChange={(event) =>
              updateContent({ text_template: event.target.value })
            }
            placeholder="Esempio: I [[blank_1]] this option because..."
            className={fieldClass}
          />
        </label>
        <p className="mt-2 text-xs font-semibold leading-5 text-cyan-900/70 dark:text-cyan-100/70">
          Inserisci ogni chiave nel testo tra doppie parentesi quadre, per
          esempio <code>[[blank_1]]</code>. Sono supportati frasi lunghe, più
          paragrafi e più spazi.
        </p>
      </div>
      {blanks.map((blank, index) => (
        ...
        >
          <div className="grid gap-3 sm:grid-cols-[10rem_minmax(0,1fr)_7rem_auto] sm:items-end">
            ...
            <label className="text-xs font-black">
              Risposte accettate, separate da |
              <input
            ...
            <label className="text-xs font-black">
              ...
              <input
                ...
                onChange={(event) =>
                  update(index, { points: Number(event.target.value) || 1 })
                }
                className={fieldClass}
              />
            </label>
  ...
  return (
    <div className="grid gap-3">
      ...
      {rubric.map((criterion, index) => (
        <div
          key={criterion.key}
          className="grid gap-3 rounded-xl border border-violet-200 bg-violet-50/40 p-4 dark:border-violet-300/20 dark:bg-violet-400/[0.05] sm:grid-cols-[10rem_minmax(0,1fr)_7rem_auto] sm:items-end"
        >
          <label className="text-xs font-black">
            Etichetta
            <input
          ...
          <label className="text-xs font-black">
            ...
            <input
              ...
              min="0.5"
              step="0.5"
              value={criterion.max_points || 1}
              onChange={(event) =>
                update(index, { max_points: Number(event.target.value) || 1 })
              }
              className={fieldClass}
            />
          </label>
          <button
            ...
            onClick={() =>
              onChange(rubric.filter((_, current) => current !== index))
            }
            className="pb-3 text-xs font-black text-red-700 underline disabled:opacity-30 dark:text-red-300"
          >
            Rimuovi
...
function WrittenEditor({ question, setQuestion }) {
  ...
  return (
    <div className="grid gap-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="text-xs font-black">
          ...
          <input
            type="number"
            min="1"
            value={content.min_words || 1}
            onChange={(event) =>
              update({ min_words: Number(event.target.value) || 1 })
            }
            className={fieldClass}
          />
        </label>
        <label className="text-xs font-black">
          ...
          <input
            type="number"
            min="1"
            value={content.max_words || 1}
            onChange={(event) =>
              update({ max_words: Number(event.target.value) || 1 })
            }
            className={fieldClass}
          />
        </label>
      ...
      <label className="text-xs font-black">
        Punti obbligatori, uno per riga
        <textarea
          rows={4}
          value={(content.required_points || []).join("\n")}
          onChange={(event) =>
            update({
              required_points: event.target.value
                .split("\n")
                .map((item) => item.trim())
                .filter(Boolean),
            })
      ...
      />
      <label className="text-xs font-black">
        Risposta modello, visibile soltanto all’admin
        <textarea
          rows={5}
...
function DialogueEditor({ question, setQuestion }) {
  ...
  const updateCharacter = (index, patch) =>
    updateContent({
      characters: characters.map((item, current) =>
        current === index ? { ...item, ...patch } : item,
      ),
    });
  const updateTurn = (index, patch) =>
    updateContent({
      turns: turns.map((item, current) =>
        current === index ? { ...item, ...patch } : item,
      ),
    });
  ...
  return (
    <div className="grid gap-5">
      ...
      <label className="text-xs font-black">
        ...
        <select
          value={content.response_mode || "written"}
          onChange={(event) =>
            updateContent({ response_mode: event.target.value })
          }
          className={fieldClass}
        >
          <option value="written">Testo per ogni turno</option>
          <option value="audio">Audio legacy (compatibilità)</option>
          <option value="audio_per_turn">Audio separato per ogni turno</option>
        </select>
      </label>
      <section>
        <div className="flex items-center justify-between">
            ...
            onClick={() =>
              updateContent({
                characters: [
                  ...characters,
                  {
                    key: `character_${characters.length + 1}`,
                    name: `Personaggio ${characters.length + 1}`,
                    description: "",
                    selectable: true,
                  },
                ],
        ...
        <div className="mt-3 grid gap-3">
          {characters.map((character, index) => (
            <div
              ...
              className="grid min-w-0 gap-3 rounded-xl border border-ink/10 p-4 dark:border-white/10 sm:grid-cols-[10rem_12rem_minmax(0,1fr)_auto_auto] sm:items-end"
            >
              <label className="text-xs font-black">
                Chiave
                <input
                  value={character.key || ""}
                  onChange={(event) =>
                    updateCharacter(index, { key: event.target.value })
                  }
                  className={fieldClass}
                />
              </label>
              ...
              <label className="flex items-center gap-2 pb-3 text-xs font-black">
                <input
                  type="checkbox"
                  checked={character.selectable !== false}
                  onChange={(event) =>
                    updateCharacter(index, { selectable: event.target.checked })
                  }
                />
                Selezionabile
              </label>
      ...
      <section>
              ...
              updateContent({
                turns: [
                  ...turns,
                  {
                    ...
                    speaker: characters[0]?.key || "",
                    text: "",
                    prompt: "",
                    learner_response: true,
                    required: true,
                    constraints: { min_seconds: 5, max_seconds: 35 },
                  },
                ],
              })
            }
        ...
        <div className="mt-3 grid gap-3">
          {turns.map((turn, index) => (
            <div
              key={`${turn.key}-${index}`}
              className="min-w-0 rounded-xl border border-ink/10 p-4 dark:border-white/10"
            >
              <div className="grid gap-3 sm:grid-cols-[10rem_12rem_auto_auto] sm:items-end">
                <label className="text-xs font-black">
                  ...
                  <input
                    value={turn.key || ""}
                    onChange={(event) =>
                      updateTurn(index, { key: event.target.value })
                    }
                    className={fieldClass}
                  />
                </label>
                ...
                <label className="flex items-center gap-2 pb-3 text-xs font-black">
                  <input
                    type="checkbox"
                    checked={Boolean(turn.learner_response)}
                    onChange={(event) =>
                      updateTurn(index, {
                        learner_response: event.target.checked,
                      })
                    }
                  />
                  Risposta studente
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                <label className="text-xs font-black">
                  Battuta fissa dell’altro personaggio
                  <textarea
                    rows={2}
                    value={turn.text || ""}
                    onChange={(event) =>
                ...
                <label className="text-xs font-black">
                  Indicazione per la battuta dello studente
                  <textarea
                    rows={2}
                    value={turn.prompt || ""}
                    onChange={(event) =>
                      updateTurn(index, { prompt: event.target.value })
                    }
                    className={fieldClass}
                  />
                </label>
              </div>
              {audioPerTurn && turn.learner_response ? (
                <DialogueTurnAudioEditor
                  turn={turn}
                  onChange={(patch) => updateTurn(index, patch)}
                />
              ) : null}
            </div>
          ))}
      ...
      <RubricEditor
        rubric={content.rubric || []}
        onChange={(rubric) => updateContent({ rubric })}
      />
    </div>
  );
}

function DialogueTurnAudioEditor({ turn, onChange }) {
  ...
  const lines = (value) =>
    String(value || "")
      ...
      .filter(Boolean);
  return (
    <div className="mt-4 grid gap-3 rounded-xl border border-violet-200 bg-violet-50/40 p-4 dark:border-violet-300/20 dark:bg-violet-400/[0.05]">
      <label className="flex items-center gap-2 text-xs font-black">
        <input
          type="checkbox"
          checked={turn.required !== false}
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="text-xs font-black">
          Suggerimento prima della registrazione
          <input
            value={turn.hint || ""}
        ...
        <label className="text-xs font-black">
          Suggerimento dopo il primo tentativo
          <input
            value={turn.retry_hint || ""}
            onChange={(event) => onChange({ retry_hint: event.target.value })}
            className={fieldClass}
          />
        </label>
      </div>
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="text-xs font-black">
          ...
          <input
            type="number"
            min="0"
            value={constraints.min_seconds ?? 0}
            onChange={(event) =>
              updateConstraints({
                min_seconds: Number(event.target.value) || 0,
              })
            }
            className={fieldClass}
          />
        ...
        <label className="text-xs font-black">
          ...
          <input
            type="number"
            min="5"
            value={constraints.max_seconds ?? 35}
            onChange={(event) =>
              updateConstraints({
                max_seconds: Number(event.target.value) || 35,
              })
            }
            className={fieldClass}
          />
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="text-xs font-black">
          ...
          <textarea
            rows={3}
            value={(constraints.required_points || []).join("\n")}
            onChange={(event) =>
              updateConstraints({ required_points: lines(event.target.value) })
            }
            className={fieldClass}
          />
        </label>
        <label className="text-xs font-black">
          Espressioni consigliate, una per riga
          <textarea
            rows={3}
            value={(constraints.recommended_language || []).join("\n")}
            onChange={(event) =>
              updateConstraints({
                recommended_language: lines(event.target.value),
              })
            }
            className={fieldClass}
          />
        ...
        <label className="text-xs font-black">
          Espressioni obbligatorie, una per riga
          <textarea
            rows={3}
            value={(constraints.required_language || []).join("\n")}
            onChange={(event) =>
              updateConstraints({
                required_language: lines(event.target.value),
              })
            }
            className={fieldClass}
          />
        ...
        <label className="text-xs font-black">
          Espressioni da evitare, una per riga
          <textarea
            rows={3}
            value={(constraints.avoid_language || []).join("\n")}
            onChange={(event) =>
              updateConstraints({ avoid_language: lines(event.target.value) })
            }
            className={fieldClass}
          />
        </label>
...
function AudioEditor({ question, setQuestion }) {
  ...
  return (
    <div className="grid gap-4">
      <label className="text-xs font-black">
        Contesto o punti da sviluppare
        <textarea
          rows={3}
          value={content.context || ""}
          onChange={(event) => update({ context: event.target.value })}
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="text-xs font-black">
          ...
          <input
            type="number"
            min="0"
            value={content.min_seconds || 0}
            onChange={(event) =>
              update({ min_seconds: Number(event.target.value) || 0 })
            }
            className={fieldClass}
          />
        </label>
        <label className="text-xs font-black">
          ...
          <input
            type="number"
            min="5"
            value={content.max_seconds || 90}
            onChange={(event) =>
              update({ max_seconds: Number(event.target.value) || 90 })
            }
            className={fieldClass}
          />
        </label>
      </div>
      <label className="flex items-center gap-3 rounded-xl border border-ink/10 p-4 text-sm font-black dark:border-white/10">
        <input
          type="checkbox"
          checked={content.allow_rerecord !== false}
          onChange={(event) => update({ allow_rerecord: event.target.checked })}
        />
        Permetti di registrare nuovamente prima della consegna
      </label>
      <RubricEditor
        rubric={content.rubric || []}
        onChange={(rubric) => update({ rubric })}
      />
      <label className="text-xs font-black">
        Trascrizione modello, solo admin
        <textarea
          rows={4}
          value={content.model_transcript || ""}
          onChange={(event) => update({ model_transcript: event.target.value })}
          className={fieldClass}
        />
      </label>
    </div>
...
function ReadingItemEditor({ item, index, onChange, onRemove, removable }) {
  ...
  return (
    <div className="rounded-xl border border-cyan-200 bg-cyan-50/35 p-4 dark:border-cyan-300/20 dark:bg-cyan-300/[0.04]">
      <div className="grid gap-3 sm:grid-cols-[9rem_12rem_7rem_auto] sm:items-end">
        <label className="text-xs font-black">
          Chiave
          <input
            value={item.key || ""}
        ...
        <label className="text-xs font-black">
          Tipo
          <select
            value={item.type || "short_answer"}
            onChange={(event) =>
              onChange({
                ...item,
                type: event.target.value,
                ...(event.target.value === "short_answer"
                  ? { accepted_answers: [""], options: undefined }
                  : {
                      options: [
                        { key: "a", text: "", is_correct: true },
                        { key: "b", text: "", is_correct: false },
            ...
            className={fieldClass}
          >
            <option value="multiple_choice">Scelta singola</option>
            <option value="multiple_select">Scelta multipla</option>
        ...
        <label className="text-xs font-black">
          ...
          <input
            ...
            value={item.points || 1}
            onChange={(event) =>
              onChange({ ...item, points: Number(event.target.value) || 1 })
            }
            className={fieldClass}
          />
        </label>
      ...
      <label className="mt-3 block text-xs font-black">
        ...
        <textarea
          ...
          value={item.prompt || ""}
          onChange={(event) =>
            onChange({ ...item, prompt: event.target.value })
          }
          className={fieldClass}
        />
      </label>
        ...
        <div className="mt-3 grid gap-2">
          {options.map((option, optionIndex) => (
            <div
              ...
              className="grid gap-2 sm:grid-cols-[auto_8rem_minmax(0,1fr)_auto] sm:items-end"
            >
              <label className="flex items-center gap-2 pb-3 text-xs font-black">
                <input
                  type={item.type === "multiple_select" ? "checkbox" : "radio"}
                  checked={Boolean(option.is_correct)}
                  onChange={(event) =>
                    onChange({
                      ...item,
                      options: options.map((currentOption, current) => ({
                        ...currentOption,
                        is_correct:
              ...
              <label className="text-xs font-black">
                ...
                <input
                  value={option.key || ""}
                  onChange={(event) =>
                    updateOption(optionIndex, { key: event.target.value })
                  }
                  className={fieldClass}
                />
              </label>
          ...
          <button
            ...
            onClick={() =>
              onChange({
                ...
                options: [
                  ...options,
                  {
                    key: String.fromCharCode(97 + options.length),
                    text: "",
                    is_correct: false,
                  },
                ],
            ...
            className="justify-self-start text-xs font-black text-cyan-800 underline dark:text-cyan-200"
          >
            Aggiungi opzione
          </button>
        </div>
  ...
  return (
    <div className="grid gap-3">
      {mappings.map((mapping, index) => (
        <div
          key={index}
          className="grid gap-3 rounded-xl border border-violet-200 bg-violet-50/50 p-4 dark:border-violet-300/20 dark:bg-violet-400/[0.06] sm:grid-cols-[minmax(0,1fr)_15rem_auto] sm:items-end"
        >
          <label className="text-xs font-black">
            Risposte, separate da |
            <input
              value={(mapping.matches || []).join(" | ")}
              onChange={(event) =>
                setQuestion({
                  ...question,
                  diagnostics: {
                    ...question.diagnostics,
                    answer_error_mappings: mappings.map((item, current) =>
                      current === index
                        ? {
                            ...item,
              ...
              onChange={(event) =>
                setQuestion({
                  ...
                  diagnostics: {
                    ...
                    answer_error_mappings: mappings.map((item, current) =>
                      current === index
                        ? { ...item, error_code: event.target.value }
                        : item,
                    ),
                  },
                })
          ...
          <button
            ...
            onClick={() =>
              setQuestion({
                ...
                diagnostics: {
                  ...question.diagnostics,
                  answer_error_mappings: mappings.filter(
                    (_, current) => current !== index,
                  ),
                },
              })
            }
            className="pb-3 text-xs font-black text-red-700 underline dark:text-red-300"
          >
            Rimuovi
          </button>
        </div>
...
export default function AdminExerciseQuestionEditorV2() {
  ...
  function applyFilters(list) {
    ...
    return list.filter((item) => {
      ...
      if (
        !["all", "pending"].includes(catalogStatus) &&
        item.status !== catalogStatus
      )
        return false;
      if (catalogLevel !== "all" && item.level !== catalogLevel) return false;
      if (catalogType !== "all" && item.question_type !== catalogType)
        return false;
  ...
  async function loadBase() {
    ...
    setError("");
    try {
      const [questionData, codeData] = await Promise.all([
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (
        ...
        (!requestedId && !questionData.length)
      )
        createNew();
      else if (requestedId || questionData[0]?.id)
        await openQuestion(requestedId || questionData[0].id);
    })();
    return () => {
      active = false;
    };
  ...
  function createNew() {
    ...
    const type = TYPE_GROUPS.flatMap(([, values]) =>
    ...
    setSuccess(
      type === "content_block"
        ? "Nuova pagina di spiegazione pronta."
        : "Nuova domanda pronta.",
    );
    setError("");
  }
  ...
  const catalogLevels = useMemo(
    () =>
      [...new Set(catalog.map((item) => item.level).filter(Boolean))].sort(),
    [catalog],
  );
  const catalogTypes = useMemo(
    () =>
      [
        ...new Set(catalog.map((item) => item.question_type).filter(Boolean)),
      ].sort(),
    [catalog],
  );
  const catalogTopics = useMemo(
    () =>
      [...new Set(catalog.map((item) => item.topic).filter(Boolean))].sort(),
    [catalog],
  );
  ...
  function toggleCode(code) {
    const next = testedSet.has(code)
      ? [...testedSet].filter((item) => item !== code)
      : [...testedSet, code];
    setQuestion({
      ...question,
      diagnostics: {
        ...question.diagnostics,
        tested_codes: next,
        fallback_error_code: next.includes(
          question.diagnostics.fallback_error_code,
        )
          ? question.diagnostics.fallback_error_code
          : next[0] || null,
      },
    ...
    try {
      ...
      const authoringQuestion =
        question.questionType === "word_order"
          ? {
              ...question,
              content: normalizeWordOrderAuthoringContent(question.content),
            }
          : question;
      const normalized = manual
        ? {
            ...authoringQuestion,
            grading: {
              ...authoringQuestion.grading,
              mode: "manual_review",
              weight: rubricTotal(authoringQuestion.content.rubric),
            },
          }
        : authoringQuestion;
      ...
      setSuccess(
        `${result.public_id}, versione ${result.version_number}, salvata.`,
      );
    } catch (saveError) {
      setError(
        saveError.message || "Non è stato possibile salvare la domanda.",
      );
    } finally {
      setSaving(false);
    }
  ...
  async function changeStatus(status, advance = false) {
    ...
    try {
      ...
      const currentPosition = Math.max(
        0,
        filtered.findIndex((item) => item.id === currentId),
      );
      const next = advance
        ? [
            ...filtered.slice(currentPosition + 1),
            ...filtered.slice(0, currentPosition + 1),
          ].find(canApprove)
        : null;
      if (next) {
        await openQuestion(next.id, { preserveSuccess: true });
        setSuccess(
          `${currentPublicId}: ${statusLabels[status]}. Aperta ${next.publicId}.`,
        );
      } else {
        await openQuestion(currentId, { preserveSuccess: true });
        setSuccess(`${currentPublicId}: ${statusLabels[status]}.`);
      }
    } catch (statusError) {
      setError(
          ...
          <header className="rounded-2xl border border-ink/10 bg-white p-6 shadow-soft dark:border-white/10 dark:bg-[#16211e] sm:p-8">
            ...
            <div className="mt-4 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Link
                  to="/admin/content/exercises/diagnostics"
                  className="rounded-full border border-ink/15 px-4 py-2.5 text-sm font-black text-ink dark:border-white/20 dark:text-white"
                >
                  Diagnostica
                </Link>
          ...
          <div className="mt-6 grid min-w-0 grid-cols-[minmax(0,1fr)] gap-6 xl:grid-cols-[20rem_minmax(0,1fr)] 2xl:grid-cols-[20rem_minmax(0,1fr)_28rem]">
            <aside className="order-2 min-w-0 rounded-2xl border border-ink/10 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-[#16211e] xl:order-1 xl:sticky xl:top-24 xl:max-h-[calc(100vh-7rem)] xl:overflow-y-auto">
              ...
              <input
                ...
                className={fieldClass}
                placeholder="ID, prompt, topic o tag"
              />
              <div className="mt-3 grid grid-cols-2 gap-2">
                ...
                >
                  ...
                  <option value="pending">Solo da approvare</option>
                ...
                <select
                  value={catalogTopic}
                  onChange={(event) => setCatalogTopic(event.target.value)}
                  className={compactFieldClass}
                >
              ...
              <div className="mt-4 grid gap-2">
                {filteredCatalog.map((item) => (
                  <button
                    ...
                    className={`min-w-0 rounded-xl border p-3 text-left ${question.id === item.id ? "border-moss bg-mint/25 dark:border-emerald-300/40 dark:bg-emerald-400/10" : "border-ink/10 dark:border-white/10"}`}
                  >
            ...
            <main className="order-1 grid min-w-0 grid-cols-[minmax(0,1fr)] gap-6 xl:order-2">
              <section className="min-w-0 rounded-2xl border border-ink/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-[#16211e] sm:p-7">
                <div className="flex items-end justify-between">
                  ...
                  {question.id ? (
                    <span
                      className={`rounded-full px-3 py-1.5 text-xs font-black ${statusClass(question.status)}`}
                    >
                      {statusLabels[question.status]} · v
              ...
              <section className="min-w-0 rounded-2xl border border-ink/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-[#16211e] sm:p-7">
                ...
                <div className="mt-5">
                  ...
                  {question.questionType === "dialogue_choice" ? (
                  ...
                  {["translation", "error_correction"].includes(
                    question.questionType,
                  ) ? (
                    <label className="text-xs font-black">
                      ...
                      <textarea
                        rows={5}
                        value={(question.content.accepted_answers || []).join(
                          "\n",
                        )}
                        onChange={(event) =>
                          setQuestion({
                  ...
                  {question.questionType === "word_order" ? (
                    <label className="text-xs font-black">
                      Ordine corretto, un token per riga
                      <textarea
                        rows={7}
                        value={(question.content.correct_order || []).join(
                          "\n",
                        )}
                        onChange={(event) => {
                          ...
                          setQuestion({
                            ...
                            content: {
                              ...
                              tokens: words.map((word, index) => ({
                                key:
                                  question.content.tokens?.[index]?.key ||
                                  uid("token"),
                                text: word,
                              })),
                            },
                  ...
                  {question.questionType === "written_response" ? (
                    <WrittenEditor
                      question={question}
                      setQuestion={setQuestion}
                    />
                  ) : null}
                  {question.questionType === "dialogue_roleplay" ? (
                    <DialogueEditor
                      question={question}
                      setQuestion={setQuestion}
                    />
                  ...
                  <div className="mt-6 grid gap-4 sm:grid-cols-3">
                    <label className="text-xs font-black">
                      ...
                      <input
                        ...
                        value={
                          manual
                            ? rubricTotal(question.content.rubric)
                            : question.grading.weight || 1
                        }
                        onChange={(event) =>
                          setQuestion({
                            ...question,
                    ...
                    {!manual ? (
                      <label className="text-xs font-black">
                        ...
                        <input
                          ...
                          step="0.1"
                          value={
                            question.grading.nearly_correct_multiplier ?? 0.5
                          }
                          onChange={(event) =>
                            setQuestion({
                              ...question,
                              grading: {
                                ...question.grading,
                                nearly_correct_multiplier: Number(
                                  event.target.value,
                                ),
                              },
                            })
                      ...
                      <div className="rounded-xl border border-violet-200 bg-violet-50 p-4 text-xs font-bold text-violet-800 dark:border-violet-300/20 dark:bg-violet-400/[0.06] dark:text-violet-200">
                        Correzione manuale dopo la consegna.
                      </div>
                    )}
                  </div>
                ) : null}
              ...
              {question.questionType !== "content_block" ? (
                <section className="min-w-0 rounded-2xl border border-ink/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-[#16211e] sm:p-7">
                  ...
                  <div className="mt-4 grid max-h-72 gap-2 overflow-y-auto rounded-xl border border-ink/10 p-3 dark:border-white/10">
                    {codes.map((code) => (
                      <label
                        key={code.code}
                        className="flex items-start gap-3 rounded-lg p-2 hover:bg-linen/40 dark:hover:bg-white/[0.06]"
                      >
                        <input
                          type="checkbox"
                          checked={testedSet.has(code.code)}
                  ...
                  <label className="mt-4 block text-xs font-black">
                    Fallback
                    <select
                      value={question.diagnostics.fallback_error_code || ""}
                      onChange={(event) =>
                        setQuestion({
                          ...question,
                          diagnostics: {
                            ...question.diagnostics,
                            fallback_error_code: event.target.value || null,
                          },
                        })
                      }
                      className={fieldClass}
                    >
                      ...
                      {(question.diagnostics.tested_codes || []).map((code) => (
            ...
            <aside className="order-3 min-w-0 rounded-2xl border border-ink/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-[#16211e] xl:col-start-2 2xl:col-start-3 2xl:row-start-1 2xl:sticky 2xl:top-24 2xl:max-h-[calc(100vh-7rem)] 2xl:overflow-y-auto">
              ...
              <div className="mt-5 grid gap-2">
                ...
                {question.id && canApprove(question) ? (
                  <button
                    type="button"
                    disabled={saving}
                    onClick={() => changeStatus("approved", true)}
                    className="rounded-full bg-moss px-5 py-3 text-sm font-black text-white"
                  >
                    Approva e prossima
                  </button>
                ) : null}
                {question.id && question.status !== "published" ? (
                  <button
                    type="button"
                    disabled={saving}
                ...
                {question.id && question.status !== "archived" ? (
                  <button
                    type="button"
                    disabled={saving}
