...
export default function ExercisePlayerV2() {
  ...
  const displayMode = currentSection?.feedback_timing === 'question_end'
    ? 'one_at_a_time'
  ...
  async function moveToNextQuestion() {
    ...
    if (currentSection.feedback_timing !== 'question_end' || currentQuestion.question.type === 'content_block' || currentQuestion.result) {
    ...
    try {
      ...
      setPayload(await checkExerciseQuestion({ attemptId: payload.attempt.id, attemptQuestionId: currentQuestion.id }));
    } catch (questionError) {
      setError(questionError.message || 'Non è stato possibile controllare la risposta.');
    } finally { setBusy(false); }
  }
