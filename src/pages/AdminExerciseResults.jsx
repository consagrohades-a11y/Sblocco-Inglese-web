    ...
    <article className="rounded-2xl border border-ink/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-[#16211e] sm:p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs font-semibold text-ink/45 dark:text-white/45">Versione {String(item.question_version_id).slice(0, 8)}</p>
        </div>
  ...
  return (
    <><SEO title="Risultati esercizi | Exercise Builder" description="Revisiona tentativi, risposte e diagnostica Exercise Builder." /><section className="section-shell py-8 lg:py-10"><div className="mx-auto max-w-[1600px]">
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[22rem_minmax(0,1fr)]">
        <aside className="rounded-2xl border border-ink/10 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-[#16211e] xl:sticky xl:top-24 xl:max-h-[calc(100vh-7rem)] xl:overflow-y-auto"><p className="text-xs font-black uppercase tracking-wide text-moss dark:text-emerald-300">Tentativi</p><div className="mt-3 grid gap-2"><input value={search} onChange={(event) => setSearch(event.target.value)} className={fieldClass} placeholder="Studente, esercizio o attività" /><div className="grid grid-cols-2 gap-2"><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className={fieldClass}><option value="all">Tutte</option>{Object.entries(statusLabels).map(([value,label]) => <option key={value} value={value}>{label}</option>)}</select><select value={scoreFilter} onChange={(event) => setScoreFilter(event.target.value)} className={fieldClass}><option value="all">Tutti i punteggi</option><option value="low">Sotto 60%</option><option value="medium">60–79%</option><option value="high">80%+</option></select></div></div><div className="mt-4 grid gap-2">{filteredAttempts.map((attempt) => <button key={attempt.id} type="button" onClick={() => openAttempt(attempt.id)} className={`rounded-xl border p-3 text-left transition ${detail?.attempt?.id === attempt.id ? 'border-moss bg-mint/25 dark:border-emerald-300/35 dark:bg-emerald-400/10' : 'border-ink/10 hover:bg-linen/40 dark:border-white/10 dark:hover:bg-white/[0.05]'}`}><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-black text-ink dark:text-white">{attempt.learner_name}</p><p className="mt-1 line-clamp-2 text-xs font-bold text-ink/55 dark:text-white/55">{attempt.exercise_public_id} · {attempt.exercise_title}</p></div><p className={`text-lg font-black ${scoreClass(attempt.score)}`}>{Math.round(Number(attempt.score || 0))}%</p></div><div className="mt-3 flex flex-wrap items-center justify-between gap-2"><span className={`rounded-full px-2.5 py-1 text-[0.65rem] font-black ${reviewBadge(attempt.review_status)}`}>{statusLabels[attempt.review_status]}</span><span className="text-[0.65rem] font-bold text-ink/40 dark:text-white/40">{formatDate(attempt.submitted_at || attempt.started_at)}</span></div>{Number(attempt.overridden_question_count || 0) > 0 ? <p className="mt-2 text-[0.65rem] font-black text-violet-700 dark:text-violet-300">{attempt.overridden_question_count} override docente</p> : null}</button>)}{!loading && !filteredAttempts.length ? <p className="py-6 text-sm text-ink/55 dark:text-white/55">Nessun tentativo corrispondente.</p> : null}</div></aside>

        <main className="min-w-0">
          {!detail ? <div className="rounded-2xl border border-dashed border-ink/15 bg-white p-8 text-sm text-ink/55 dark:border-white/15 dark:bg-[#16211e] dark:text-white/55">Seleziona un tentativo.</div> : <div className="grid gap-6">
            <section className="rounded-2xl border border-ink/10 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-[#16211e] sm:p-8"><div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-black uppercase tracking-wide text-moss dark:text-emerald-300">{detail.exercise?.public_id} · tentativo {detail.attempt.attempt_number}</p><h2 className="mt-2 text-3xl font-black text-ink dark:text-white">{detail.exercise?.title}</h2><p className="mt-2 text-sm font-bold text-ink/55 dark:text-white/55">{detail.attempt.learner_name} · {detail.attempt.assignment_title || 'Nessuna attività collegata'} · {formatDate(detail.attempt.submitted_at)}</p></div><div className="text-right"><p className={`text-5xl font-black ${scoreClass(detail.attempt.score)}`}>{Math.round(Number(detail.attempt.score || 0))}%</p><p className="mt-1 text-xs font-bold text-ink/45 dark:text-white/45">{Number(detail.attempt.earned_points || 0).toFixed(1)} / {Number(detail.attempt.max_points || 0).toFixed(1)} punti</p></div></div><div className="mt-6 grid gap-4 sm:grid-cols-2"><label className="text-xs font-black text-ink/60 dark:text-white/60">Stato revisione<select value={reviewStatus} onChange={(event) => setReviewStatus(event.target.value)} className={`${fieldClass} mt-2 w-full`}>{Object.entries(statusLabels).map(([value,label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="text-xs font-black text-ink/60 dark:text-white/60 sm:col-span-2">Nota generale dell’insegnante<textarea rows={3} value={teacherNote} onChange={(event) => setTeacherNote(event.target.value)} className={`${fieldClass} mt-2 w-full`} /></label></div></section>

            <ExerciseDiagnosticSummary summary={detail.attempt.result_summary?.diagnostic_summary} admin />

            {(detail.sections || []).map((section) => <section key={section.id} className="grid gap-4"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wide text-violet-700 dark:text-violet-300">Sezione {section.sequence_index + 1}</p><h2 className="mt-1 text-2xl font-black text-ink dark:text-white">{section.title}</h2></div><p className="text-sm font-black text-ink/55 dark:text-white/55">{Number(section.earned_points || 0).toFixed(1)} / {Number(section.max_points || 0).toFixed(1)}</p></div>{(section.questions || []).map((item) => <QuestionReviewCard key={item.id} item={item} review={reviews[item.id]} onChange={(patch) => updateReview(item.id, patch)} />)}</section>)}

            <div className="sticky bottom-4 z-20 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-ink/10 bg-white/95 p-4 shadow-xl backdrop-blur dark:border-white/10 dark:bg-[#16211e]/95"><p className="text-xs font-bold text-ink/50 dark:text-white/50">{Object.values(reviews).filter((review) => review.dirty).length} domande modificate</p><button type="button" disabled={saving} onClick={saveReview} className="rounded-full bg-ink px-6 py-3 text-sm font-black text-white disabled:opacity-40 dark:bg-emerald-300 dark:text-[#102019]">{saving ? 'Salvataggio...' : 'Salva revisione e ricalcola'}</button></div>
          </div>}
        </main>
      </div>
    </div></section></>
