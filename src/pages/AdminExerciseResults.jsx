...
function attemptBadge(attempt) {
  if (attempt?.status !== 'submitted') return 'bg-sky-100 text-sky-900 dark:bg-sky-400/15 dark:text-sky-200';
  return reviewBadge(attempt?.review_status);
}

function attemptStatusLabel(attempt) {
  if (attempt?.status !== 'submitted') return attemptStatusLabels[attempt?.status] || 'In corso';
  return statusLabels[attempt?.review_status] || attempt?.review_status || 'Inviato';
}
...
function QuestionReviewCard({ item, review, onChange, reviewEnabled = true }) {
      ...
      <div className="mt-5 grid gap-4 rounded-xl border border-clay/20 bg-clay/[0.045] p-4 dark:border-coral/20 dark:bg-coral/[0.06] sm:grid-cols-[12rem_9rem_minmax(0,1fr)]">
        <label className="text-xs font-black text-ink/60 dark:text-white/60">Esito docente
          <select
            ...
            disabled={!reviewEnabled || review.clearOverride}
...
export default function AdminExerciseResults() {
  ...
  const filteredAttempts = useMemo(() => {
    ...
    return attempts.filter((attempt) => {
      if (statusFilter === 'in_progress' && attempt.status === 'submitted') return false;
      if (statusFilter !== 'all' && statusFilter !== 'in_progress' && (attempt.status !== 'submitted' || attempt.review_status !== statusFilter)) return false;
  ...
  async function saveReview(targetStatus = null) {
    if (!detail?.attempt?.id || detail.attempt.status !== 'submitted') return;
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[22rem_minmax(0,1fr)]">
        ...
        <main className="min-w-0">
          {!detail ? <div className="rounded-2xl border border-dashed border-ink/15 bg-white p-8 text-sm text-ink/65 dark:border-white/15 dark:bg-surface-900 dark:text-white/65">Seleziona un tentativo.</div> : <div className="grid gap-6">
            <section className="rounded-2xl border border-ink/10 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-surface-900 sm:p-8"><div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-bold uppercase tracking-wide text-clay dark:text-coral">{detail.exercise?.public_id} · tentativo {detail.attempt.attempt_number}</p><h2 className="mt-2 text-3xl font-black text-ink dark:text-white">{detail.exercise?.title}</h2><p className="mt-2 text-sm font-bold text-ink/65 dark:text-white/65">{detail.attempt.learner_name} · {detail.attempt.assignment_title || 'Nessuna attività collegata'} · {formatDate(detail.attempt.submitted_at || detail.attempt.started_at)}</p>{getNote(detail.attempt.learner_id) ? <p className="mt-1 text-xs font-bold leading-5 text-clay dark:text-coral">{getNote(detail.attempt.learner_id)}</p> : null}</div><div className="text-right">{detail.attempt.status !== 'submitted' ? <><p className="text-xl font-black text-sky-800 dark:text-sky-200">Attività in corso</p><p className="mt-1 max-w-sm text-xs font-bold leading-5 text-ink/60 dark:text-white/60">Le risposte sotto sono il progresso autosalvato. Lo studente non ha ancora inviato l&apos;esercizio.</p></> : detail.attempt.score === null || detail.attempt.score === undefined ? <><p className="text-xl font-black text-clay dark:text-coral">Punteggio in attesa</p><p className="mt-1 text-xs font-bold text-ink/60 dark:text-white/60">L&apos;esercizio è stato inviato e richiede review manuale.</p></> : <><p className={`text-5xl font-black ${scoreClass(detail.attempt.score)}`}>{Math.round(Number(detail.attempt.score))}%</p><p className="mt-1 text-xs font-bold text-ink/60 dark:text-white/60">{Number(detail.attempt.earned_points || 0).toFixed(1)} / {Number(detail.attempt.max_points || 0).toFixed(1)} punti</p></>}</div></div><div className="mt-6 grid gap-4 sm:grid-cols-2"><div className="text-xs font-black text-ink/60 dark:text-white/60">{detail.attempt.status === 'submitted' ? 'Stato revisione' : 'Stato tentativo'}<div className="mt-2 flex min-h-11 items-center rounded-xl border border-ink/10 bg-linen/50 px-4 dark:border-white/10 dark:bg-white/[0.04]"><span className={`rounded-full px-3 py-1.5 text-xs font-black ${attemptBadge(detail.attempt)}`}>{attemptStatusLabel(detail.attempt)}</span></div></div><label className="text-xs font-black text-ink/60 dark:text-white/60 sm:col-span-2">Nota generale dell’insegnante<textarea rows={3} value={teacherNote} disabled={detail.attempt.status !== 'submitted'} onChange={(event) => setTeacherNote(event.target.value)} className={`${fieldClass} mt-2 w-full disabled:cursor-default disabled:opacity-60`} /></label></div></section>
