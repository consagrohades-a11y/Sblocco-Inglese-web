        ...
        <div className="mx-auto max-w-7xl">
          <header className="rounded-2xl border border-ink/10 bg-white p-6 shadow-soft dark:border-white/10 dark:bg-[#16211e] sm:p-8">
            ...
            <div className="mt-4 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
              ...
              <div className="flex flex-wrap gap-2"><Link to="/admin/content/exercises" className="rounded-full border border-ink/15 bg-white px-4 py-2.5 text-sm font-black text-ink dark:border-white/20 dark:bg-white/10 dark:text-white">Importa JSON</Link><Link to="/admin/content/exercises/review" className="rounded-full bg-ink px-4 py-2.5 text-sm font-black text-white dark:bg-emerald-300 dark:text-[#102019]">Revisioni</Link></div>
            </div>
          </header>
          ...
          {loading ? <div className="mt-6 rounded-2xl border border-ink/10 bg-white p-6 text-sm font-bold text-ink/60 dark:border-white/10 dark:bg-[#16211e] dark:text-white/60">Caricamento libreria...</div> : null}
                ...
                {items.map((item) => (
                  <article key={item.id} className="grid gap-4 px-5 py-5 lg:grid-cols-[7rem_minmax(0,1.4fr)_8rem_9rem_minmax(18rem,1fr)] lg:items-center">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      {item.status !== 'published' ? <button type="button" disabled={busyId === item.id} onClick={() => changeStatus(item, 'published')} className="rounded-full bg-ink px-3.5 py-2 text-xs font-black text-white disabled:opacity-50 dark:bg-emerald-300 dark:text-[#102019]">Pubblica</button> : null}
