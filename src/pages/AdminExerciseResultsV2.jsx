    ...
    <article className={`rounded-2xl border p-5 shadow-sm dark:bg-[#16211e] sm:p-6 ${pending ? 'border-violet-300 bg-violet-50/40 dark:border-violet-300/25' : 'border-ink/10 bg-white dark:border-white/10'}`}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs font-semibold text-ink/45 dark:text-white/45">{type} · massimo {maximum} punti</p>
        </div>
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            {rubric.map((criterion) => (
              <div key={criterion.key} className="rounded-lg bg-white/75 p-3 dark:bg-white/[0.05]">
                ...
                {criterion.description ? <p className="mt-1 text-xs font-semibold leading-5 text-ink/50 dark:text-white/50">{criterion.description}</p> : null}
              </div>
            ))}
          </div>
        </div>
      ...
      {!pending ? (
        <label className="mt-3 flex items-center gap-2 text-xs font-black text-ink/55 dark:text-white/55">
