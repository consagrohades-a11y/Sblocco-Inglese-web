    ...
    <>
      ...
      <TrainerLayout showNav={!locked}>
        <div className="mx-auto max-w-5xl">
          <div className="relative mb-5 flex flex-wrap items-center justify-between gap-4 overflow-hidden rounded-2xl border border-moss/15 bg-gradient-to-br from-white via-white to-mint/35 p-5 shadow-sm dark:border-white/10 dark:from-surface-900 dark:via-surface-900 dark:to-[#17352c] sm:p-6">
            <div aria-hidden="true" className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-moss via-coral to-[#ffc457]" />
            <div><span className="inline-flex items-center gap-2 rounded-full border border-moss/15 bg-mint/55 px-3 py-1.5 text-xs font-bold uppercase tracking-[0.12em] text-moss dark:border-emerald-300/20 dark:bg-emerald-300/10 dark:text-mint"><Sparkles className="h-4 w-4" />Pratica sincronizzata</span><h1 className="mt-3 text-3xl font-black text-ink dark:text-white sm:text-4xl">Esercizi dalle tue card</h1>{session && !finished ? <p className="mt-2 text-sm font-semibold text-ink/60 dark:text-white/60">Completa una domanda alla volta. Il risultato viene salvato automaticamente.</p> : null}</div>
          ...
          {!session ? (
            <section className="rounded-xl border border-ink/10 bg-white p-5 shadow-soft dark:border-white/10 dark:bg-white/[0.06] sm:p-7">
              ...
              <div className="mt-6 flex flex-wrap items-end justify-between gap-4 border-t border-ink/10 pt-5 dark:border-white/10">
                ...
                <div className="text-right"><p className="mb-2 text-xs font-bold text-ink/65 dark:text-white/65">{loading ? 'Caricamento...' : `${filteredCards.length} card uniche e ${availableQuestions} domande disponibili`}</p><button type="button" onClick={startSession} disabled={loading || Boolean(loadError) || !filteredCards.length || !modes.length || assignmentConfigurationMismatch} className="focus-ring rounded-full bg-ink px-6 py-3 text-sm font-black text-white shadow-sm transition hover:bg-moss disabled:cursor-not-allowed disabled:bg-ink/25 disabled:text-ink/60 dark:bg-mint dark:text-ink dark:hover:bg-white dark:disabled:bg-white/15 dark:disabled:text-white/35">{loading ? <Loader2 className="mr-2 inline h-4 w-4 animate-spin" /> : null}Inizia</button></div>
              </div>
            ...
            <section className="relative overflow-hidden rounded-2xl border border-moss/15 bg-gradient-to-br from-white via-white to-[#fffaf0] p-5 shadow-soft dark:border-white/10 dark:from-[#14201c] dark:via-[#14201c] dark:to-[#292019] sm:p-8">
              <div aria-hidden="true" className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-moss via-coral to-[#ffc457]" />
              <div className="flex flex-wrap items-center justify-between gap-3 text-xs font-bold uppercase tracking-[0.1em]"><span className="inline-flex items-center gap-2 rounded-full border border-coral/15 bg-[#fff1df] px-3 py-1.5 text-[#9a3f29] dark:border-coral/25 dark:bg-coral/15 dark:text-[#ffc7b0]"><Sparkles className="h-3.5 w-3.5" />{current.instruction}</span><span className="rounded-full border border-moss/15 bg-mint/55 px-3 py-1.5 text-moss dark:border-emerald-300/20 dark:bg-emerald-300/10 dark:text-mint">Domanda {index + 1} di {session.length}</span></div>
