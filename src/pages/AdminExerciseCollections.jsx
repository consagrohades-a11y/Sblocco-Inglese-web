...
function detailToCollection(detail) {
  return {
    ...
    status: detail.catalog_status || 'active',
...
export default function AdminExerciseCollections() {
  ...
  async function changeStatus(status) {
    ...
    try {
      ...
      setSuccess(status === 'archived' ? 'Collection archiviata.' : 'Collection riattivata.');
      await loadBase();
  ...
  return <><SEO title="Collections | Exercise Builder" description="Raggruppa domande, pool ed esercizi senza modificarli." /><section className="section-shell py-8 lg:py-10"><div className="mx-auto max-w-[1500px]">
    <header className="rounded-2xl border border-ink/10 bg-white p-6 shadow-soft dark:border-white/10 dark:bg-[#16211e] sm:p-8"><span className="eyebrow">Exercise Builder</span><div className="mt-4 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><h1 className="text-3xl font-black text-ink dark:text-white sm:text-4xl">Collections</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-ink/65 dark:text-white/65">Raggruppa domande, pool ed esercizi per corso, argomento, studente o obiettivo. La Collection non modifica né duplica i contenuti.</p></div><div className="flex flex-wrap gap-2"><Link to="/admin/content/exercises/questions" className="rounded-full border border-ink/15 bg-white px-4 py-2.5 text-sm font-black text-ink dark:border-white/20 dark:bg-white/10 dark:text-white">Question Bank</Link><Link to="/admin/content/exercises/pools" className="rounded-full border border-ink/15 bg-white px-4 py-2.5 text-sm font-black text-ink dark:border-white/20 dark:bg-white/10 dark:text-white">Pool Builder</Link><button type="button" onClick={createNew} className="rounded-full bg-ink px-4 py-2.5 text-sm font-black text-white dark:bg-emerald-300 dark:text-[#102019]">Nuova Collection</button></div></div></header>
