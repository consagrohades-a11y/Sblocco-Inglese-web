...
const fieldClass = 'w-full rounded-xl border border-ink/15 bg-white px-3.5 py-2.5 text-sm font-semibold text-ink outline-none focus:border-moss focus:ring-4 focus:ring-mint/30 dark:border-white/20 dark:bg-surface-800 dark:text-white dark:focus:border-emerald-300 dark:focus:ring-emerald-400/15';
          ...
          <header className="rounded-2xl border border-ink/10 bg-white p-6 shadow-soft dark:border-white/10 dark:bg-surface-900 sm:p-8">
            ...
            <div className="mt-5 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button type="button" disabled={loading} onClick={() => downloadDiagnosticTaxonomy(buildDiagnosticTaxonomyExport(registry), 'diagnostic-taxonomy-current.json')} className="inline-flex items-center gap-2 rounded-full bg-ink px-4 py-2.5 text-sm font-black text-white disabled:opacity-50 dark:bg-emerald-300 dark:text-surface-950"><Download className="h-4 w-4" />Esporta tassonomia corrente</button>
              </div>
            </div>
          </header>
          ...
          <div className="mt-6 grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_24rem]">
            <section className="min-w-0 rounded-2xl border border-ink/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-surface-900 sm:p-7">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div><p className="text-xs font-bold uppercase tracking-wide text-moss dark:text-emerald-300">Sorgente JSON</p><h2 className="mt-1 text-2xl font-black text-ink dark:text-white">Incolla o carica il file</h2></div>
              ...
              <textarea value={jsonText} onChange={(event) => { setJsonText(event.target.value); setValidation(null); }} spellCheck={false} className="mt-4 min-h-[38rem] w-full rounded-xl border border-ink/15 bg-surface-800 p-4 font-mono text-xs leading-6 text-emerald-50 outline-none focus:border-emerald-400" />
            </section>

            <aside className="rounded-2xl border border-ink/10 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-surface-900 xl:sticky xl:top-24">
              <p className="text-xs font-bold uppercase tracking-wide text-violet-700 dark:text-violet-300">Importazione</p>
