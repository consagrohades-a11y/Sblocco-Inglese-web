...
if (noAudio?.question?.content?.audio) {
...
const migration = await readFile(path.join(root, 'supabase/migrations/20260812115500_native_listening_comprehension.sql'), 'utf8');
for (const marker of [
...
console.log('Native listening comprehension validation passed: template, bridge, audio contract, renderer and database migration are aligned.');
